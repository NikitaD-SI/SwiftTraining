...
class SurveysTableViewController: UITableViewController {
    ...
    override func viewWillAppear(_ animated: Bool) {
        ...
        dataModelHandle.pullData(UpdateScreenWith: {(responses : [SurveyModel]) -> Void in
            for entry in responses
            {
                let dateFormatter = DateFormatter()
                
                self.listData[MonthType.enumFrom(Date: entry.dateOfSurvey)]?.append(entry)
            }
    ...
    override func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell
    {
        ...
        cell?.detailTextLabel?.text = rowData.dateOfSurvey.description
        return cell!
    }
